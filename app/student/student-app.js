...
function canStartExam() {
  ...
  if (name.length <= 15) {
    customDialog("../../utils/logo.png", "alert", "Aviso:", 'El nombre completo debe tener al menos 16 caracteres.');
    return false;
  }
  if (code.length <= 7) {
    customDialog(".../../utils/logo.png", "alert", "Aviso:", 'El código de aplicación debe tener al menos 8 caracteres.');
    return false;
  }
...
async function startExam() {
  ...
  if (Array.isArray(allowedGroups) && allowedGroups.length > 0 && !allowedGroups.includes(selectedGradeText)) {
    setTimeout(() => customDialog("../../utils/logo.png", "alert", "Aviso:", 'La prueba no está disponible para el grupo/curso seleccionado.'), 4500);
    return;
  }

  // validar ventana de la prueba (fecha + hora)
  if (!isTestOpen(currentTest)) {
    setTimeout(() => customDialog("../../utils/logo.png", "alert", "Aviso:", 'La prueba no está disponible en este momento. Verifica la fecha y hora de apertura.'), 4500);
    return;
  }
...
function attachAntiCheatListeners() {
  ...
  function handleAntiCheatEvent(kind, warningMessage) {
    ...
    const entry = recordEvent(kind);
    
    // Verificar si la prueba tiene sistema anti-cheat con penalización
    const hasAntiCheat = currentTest && currentTest.antiCheat === true;
    const pointsPerCheat = currentTest && currentTest.pointsPerCheat ? currentTest.pointsPerCheat : 0;
    
    if (hasAntiCheat && pointsPerCheat > 0) {
      // Acumular penalización
      cheatPenalty += pointsPerCheat;
      
      // Mostrar advertencia con información de penalización
      const warningWithPenalty = `${warningMessage}\n\nSe aplicará una penalización de ${pointsPerCheat.toFixed(1)} puntos por este intento.`;
      
      if (totalCheatEvents <= 3) {
        customDialog("../../utils/logo.png", "alert", "Advertencia:", warningWithPenalty);
      } else if (totalCheatEvents === 4) {
        customDialog("../../utils/logo.png", "alert", "Advertencia Final:", 
          `Has acumulado ${totalCheatEvents} advertencias. Se aplicará una penalización total de ${cheatPenalty.toFixed(1)} puntos a tu calificación final.`);
      }
    } else {
      // Comportamiento antiguo (bloqueo después de 3 eventos)
      if (totalCheatEvents <= 2) {
        customDialog("../../utils/logo.png", "alert", "Aviso:", warningMessage);
      }
    }
...
function renderQuestion() {
  ...
  let inner = q.context ? `<p style="margin-top:2.5px;">${escapeHtml(q.context)}</p>` : '';
  ...
  inner += `<h3>${currentQuestionIndex + 1}. ${escapeHtml(q.title)}</h3>`;
...
async function finishExam() {
  ...
  setTimeout(async () => {
    ...
    let totalScore = 0;
    let originalScore = 0;  // Puntuación sin penalizaciones
    const details = [];
    ...
    if (currentTest && currentTest.antiCheat === true && cheatPenalty > 0) {
      totalScore = Math.max(0, totalScore - cheatPenalty);
      totalScore = Math.round(totalScore * 1000) / 1000;
    }
    ...
    if (currentTest.evaluativa !== false) {
      if (cheatPenalty > 0) {
        summaryText += `Puntaje Obtenido: ${originalScore.toFixed(2)}\n`;
        summaryText += `Penalización por fraude: -${cheatPenalty.toFixed(2)}\n`;
        summaryText += `Puntaje final: ${totalScore.toFixed(2)}\n`;
      } else {
        summaryText += `Puntaje: ${totalScore.toFixed(2)}\n`;
      }
    }
    ...
    try {
      ...
      const resultObj = {
        ...
        timestamp: new Date().toISOString(),
        scoreOriginal: originalScore,    
        scoreFinal: totalScore,         
        penalty: cheatPenalty,          
        details,
        cheatLogs,
        cheatEvents: totalCheatEvents,
        resultCode
      };
        ...
        if (copyBtn) {
          copyBtn.addEventListener('click', () => {
            navigator.clipboard?.writeText(resultCode).then(() => {
              ...
              try { document.execCommand('copy'); showSnackbar("Código copiado al portapapeles", { type: "success" }); } catch(e){ customDialog("../utils/logo.png", "prompt", "Copia manual:", "", { placeholder: "Copia manual", defaultValue: ta.value }); }
              ta.remove();
            });
          });
        }
...
function downloadResults(resultObj) {
  ...
  if (resultObj && (!resultObj.details || resultObj.details.length === 0)) {
    if (!customDialog("../../utils/logo.png", "confirm", "Confirmar", '¿Estás seguro de que deseas descargar tus respuestas actuales en formato JSON?')) {
      return;
    }
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  if (btnSearch) btnSearch.addEventListener('click', () => {
    ...
    if (!/^\d{11}$/.test(code)) { customDialog("../../utils/logo.png", "alert", "Aviso:", 'Ingresa un código válido de 11 dígitos.'); return; }
